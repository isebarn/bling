...
function Layout1(story, chapterId, shaddowHeight) {
    ...
    <View style={styles.container}>
     ...
     <LinearGradient colors={['transparent', ...Array(shaddowHeight).fill('black')]}
          ...
          start={{ x: 0, y: 0.5 }}
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,.2)'}}>
              <View style={{ flex: 1}}></View>
              <View style={{ flex: 1}}></View>
              <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center'}}>
                <Text style={{fontSize: 30, fontFamily: 'Kimberley', color: "#BDFDFD", justifyContent: 'center'}}>{chapter.name}</Text>
                <RenderHtml contentWidth={200} source={{ html: chapter.content }} tagsStyles={styles.tagsStyles} systemFonts={systemFonts}/>
                <View style={{ flex: 1, alignSelf: 'stretch', justifyContent: 'center', alignItems: 'center', marginVertical: 20}}>
                  ...
                  {chapter.choices.map((choice) => (
                    <LinearGradient key={choice.id} colors={['#FEA900', '#FE4200']} style={[styles.buttonStyle, styles.roundButton]}>
                      <Text style={{ fontSize: 20, color: 'black', backgroundColor: 'transparent', fontSize: 20, fontFamily: 'Kimberley'}} onPress={(item) => {
                        return Layout1(story, choice.chapter.id, shaddowHeight)
                      }}>{choice.text}</Text>
                    </LinearGradient>
                  ))} 
...
export default function App() {
  ...
  return Layout1(data[0], data[0].chapters[0].id, 2);
